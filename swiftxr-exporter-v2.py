...
class SwiftXRConfirm(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        # return context.window_manager.invoke_confirm(self, message="Are you sure you want to perform this action?", title="Confirmation", icon='QUESTION', options={'REGISTER', 'INTERNAL'})
...
class SwiftXRExport(bpy.types.Operator, ExportHelper):
    ...
    immersive_mode: EnumProperty(
        ...
        items=(('3D', "3D", "View in 3D on the Web"),
               ('AR', "3D & Augmented Reality (AR)", "View in 3D on the Web and in AR"),
    ...
    #swiftxr_api_key = 'SWF.Pebhdjq5tPwqUQ.Y9AFSWi1X0Q_tMgrglFLv8ieHjGIczXadvtJYIbAh7w'
    ...
    #     if not swiftxr_api_key:
    #         self.report(
    #             {"ERROR"}, "API key not set. Please set your API key in the plugin preferences.")
    #         return {"CANCELLED"}
    ...
    #     if site_name:
    #         self.swiftxr_site_name = site_name
...
class SWIFTXR_PT_export_main(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.prop(operator, "swiftxr_api_key")
        row = layout.row(align=True)
...
def menu_func_export(self, context):
    self.layout.operator(SwiftXRExport.bl_idname, text="SwiftXR (3D/AR/VR) Viewer")
